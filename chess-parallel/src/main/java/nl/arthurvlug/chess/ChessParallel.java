...
//		distributedCalculation();
	...
	private static void memoryCalculation() {
		...
		String bestMove = bestMove(initialPosition).toString();
	...
	private static void distributedCalculation() throws IOException {
		...
		MemCollection<String> bestMoveCollection = new MemCollection<>(Arrays.asList(bestMove(initialPosition).toString()));
	...
	private static Position bestMove(final PCollection<String> firstNode) {
